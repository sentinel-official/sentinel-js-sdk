...
export const EventCreate: MessageFns<EventCreate> = {
  encode(message: EventCreate, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (!message.leaseId.equals(Long.UZERO)) {
      writer.uint32(8).uint64(message.leaseId.toString());
    }
    ...
    if (!message.maxHours.equals(Long.ZERO)) {
      writer.uint32(32).int64(message.maxHours.toString());
    }
    if (message.price !== "") {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1: {
          ...
          message.leaseId = Long.fromString(reader.uint64().toString(), true);
          continue;
        }
        case 2: {
          if (tag !== 18) {
  ...
  fromJSON(object: any): EventCreate {
    return {
      leaseId: isSet(object.leaseId)
        ...
        ? Long.fromValue(object.lease_id)
        : Long.UZERO,
