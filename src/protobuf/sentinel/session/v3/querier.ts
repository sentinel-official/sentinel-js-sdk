...
export const QuerySessionResponse = {
  encode(message: QuerySessionResponse, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.session !== undefined) {
      Any.encode(message.session, writer.uint32(10).fork()).ldelim();
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          message.session = Any.decode(reader, reader.uint32());
          continue;
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
  ...
  fromJSON(object: any): QuerySessionResponse {
    return { session: isSet(object.session) ? Any.fromJSON(object.session) : undefined };
  },

  toJSON(message: QuerySessionResponse): unknown {
    const obj: any = {};
    if (message.session !== undefined) {
      obj.session = Any.toJSON(message.session);
    }
    return obj;
  },
  ...
  fromPartial(object: DeepPartial<QuerySessionResponse>): QuerySessionResponse {
    ...
    message.session = (object.session !== undefined && object.session !== null)
      ? Any.fromPartial(object.session)
      : undefined;
    return message;
  },
};
...
export const QueryServiceServiceName = "sentinel.session.v3.QueryService";
