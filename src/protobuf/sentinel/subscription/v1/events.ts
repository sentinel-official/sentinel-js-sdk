...
export const EventAddQuota: MessageFns<EventAddQuota> = {
  encode(message: EventAddQuota, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): EventAddQuota {
...
export const EventSubscribe: MessageFns<EventSubscribe> = {
  encode(message: EventSubscribe, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (!message.plan.equals(Long.UZERO)) {
      writer.uint32(24).uint64(message.plan.toString());
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 3: {
          ...
          message.plan = Long.fromString(reader.uint64().toString(), true);
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
...
export const EventUpdateQuota: MessageFns<EventUpdateQuota> = {
  encode(message: EventUpdateQuota, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (!message.id.equals(Long.UZERO)) {
      writer.uint32(8).uint64(message.id.toString());
    }
    if (message.address !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): EventUpdateQuota {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
