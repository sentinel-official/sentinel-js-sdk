...
export const Subscription: MessageFns<Subscription> = {
  encode(message: Subscription, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (!message.plan.equals(Long.UZERO)) {
      writer.uint32(48).uint64(message.plan.toString());
    }
    if (message.denom !== "") {
      writer.uint32(58).string(message.denom);
    }
    if (message.expiry !== undefined) {
      Timestamp.encode(toTimestamp(message.expiry), writer.uint32(66).fork()).join();
    }
    if (message.free !== "") {
    ...
    if (message.statusAt !== undefined) {
      Timestamp.encode(toTimestamp(message.statusAt), writer.uint32(90).fork()).join();
    }
    return writer;
  },
