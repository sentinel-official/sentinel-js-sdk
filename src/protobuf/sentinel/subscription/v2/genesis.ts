...
export const GenesisSubscription: MessageFns<GenesisSubscription> = {
  encode(message: GenesisSubscription, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.subscription !== undefined) {
      Any.encode(message.subscription, writer.uint32(10).fork()).join();
    }
    for (const v of message.allocations) {
      Allocation.encode(v!, writer.uint32(18).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): GenesisSubscription {
...
export const GenesisState: MessageFns<GenesisState> = {
  encode(message: GenesisState, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.subscriptions) {
      GenesisSubscription.encode(v!, writer.uint32(10).fork()).join();
    }
