...
export const QueryNodesForPlanRequest: MessageFns<QueryNodesForPlanRequest> = {
  encode(message: QueryNodesForPlanRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (!message.id.equals(Long.UZERO)) {
      writer.uint32(8).uint64(message.id.toString());
    }
    if (message.pagination !== undefined) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): QueryNodesForPlanRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
...
export const QueryNodesForPlanResponse: MessageFns<QueryNodesForPlanResponse> = {
  encode(message: QueryNodesForPlanResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): QueryNodesForPlanResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
...
export class QueryServiceClientImpl implements QueryService {
  ...
  QueryNodesForPlan(request: QueryNodesForPlanRequest): Promise<QueryNodesForPlanResponse> {
    ...
    return promise.then((data) => QueryNodesForPlanResponse.decode(new BinaryReader(data)));
  }
}
